 ...
 * sensor 3 (farthest from the stop-rail, used for damper-off, i.e. note-off)
...
void checkNotes(int midiNote) {
  ...
  checkNote(midiNote, digitalReadFast(note1_3) == LOW);
  checkNote(midiNote + 1, digitalReadFast(note2_3) == LOW);
  checkNote(midiNote + 2, digitalReadFast(note3_3) == LOW);
  checkNote(midiNote + 3, digitalReadFast(note4_3) == LOW);
  checkNote(midiNote + 4, digitalReadFast(note5_3) == LOW);
}
